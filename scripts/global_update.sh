...
date +"* Start Creating aa-ac.sqlite Data Base (%c) using $SQLITE" | tee $LOG
...
if [ $? -ne "0" ]; then 
  echo "    Creation of aa-ac.sqlite failed - see log file for details" | tee $LOG
else 
  SIZE=`du -k $ARCHIVE/db/aa-ac.sqlite | cut -f1`
  echo "    Created aa-ac.sqlite successfully ($SIZE kb)" | tee $LOG
fi

if [ -f "$ARCHIVE"/db/aa-ac.sqlite ]; then 
  date +'* Start Compressing aa-ac.sqlite Data Base (%c)' | tee $LOG
  rm -f $ARCHIVE/db/aa-ac.sqlite.bz2
  pbzip2 -f "$ARCHIVE"/db/aa-ac.sqlite 
  if [ $? -ne "0" ]; then 
    echo "    Creation of aa-ac.sqlite.bz2 failed"  | tee $LOG
  else 
    SIZE=`du -k $ARCHIVE/db/aa-ac.sqlite.bz2 | cut -f1`
    echo "    Created aa-ac.sqlite.bz2 successfully ($SIZE kb)" | tee $LOG
  fi
fi

date +"* Start Creating full.sqlite Data Base (%c) using $SQLITE" | tee $LOG
...
if [ $? -ne "0" ]; then 
  echo "    Creation of full.sqlite failed - see log file for details" | tee $LOG
else 
  SIZE=`du -k $ARCHIVE/db/full.sqlite | cut -f1`
  echo "    Created full.sqlite successfully ($SIZE kb)" | tee $LOG
fi

if [ -f "$ARCHIVE"/db/full.sqlite ]; then 
  date +'* Start Compressing full.sqlite Data Base (%c)' | tee $LOG
  rm -f $ARCHIVE/db/full.sqlite.bz2
  pbzip2 -f "$ARCHIVE"/db/full.sqlite 
  if [ $? -ne "0" ]; then 
    echo "    Creation of full.sqlite.bz2 failed" | tee $LOG
  else 
    SIZE=`du -k $ARCHIVE/db/full.sqlite.bz2 | cut -f1`
    echo "    Created full.sqlite.bz2 successfully ($SIZE kb)" | tee $LOG
  fi
fi
...
date +'* Update Finished (%c)' | tee $LOG
